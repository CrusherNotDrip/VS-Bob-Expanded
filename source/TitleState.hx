...
class TitleState extends MusicBeatState
{
	...
	private var camTitle:FlxCamera;
	private var camAchievement:FlxCamera;

	public static var titleTime:Float = 0; //for why achievement
	...
	override public function create():Void
	{
		...
		FlxG.cameras.reset(camTitle);

		camAchievement = new FlxCamera();
		camAchievement.bgColor.alpha = 0;
		FlxG.cameras.add(camAchievement, false);

		FlxG.cameras.setDefaultDrawTarget(camTitle, true);
	...
	function startIntro()
	{
		...
		logoBl.scale.set(titleJSON.titlescalex, titleJSON.titlescaley);
		logoBl.updateHitbox();
	...
	override function update(elapsed:Float)
	{
		...
		if (titleTime >= 3000) {
			var achieveID:Int = Achievements.getAchievementIndex('title_one_minute');
				...
				if(allowedKeys.contains(keyName)) {
					...
					for (wordRaw in easterEggKeys)
					{
						...
						if (easterEggKeysBuffer.contains(word))
						{
							...
							if(freeplay.FreeplayState.vocals != null)
							{
								freeplay.FreeplayState.vocals.fadeOut();
							}
							closedState = true;
							transitioning = true;
							playJingle = true;
	...
	function giveAchievement() {
		add(new AchievementObject('title_one_minute', camAchievement));
		FlxG.sound.play(Paths.sound('confirmMenu'), 0.7);
		trace('Giving achievement "title_one_minute"');
	}
	#end
